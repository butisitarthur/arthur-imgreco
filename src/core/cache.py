...
class CacheService:
    ...
    async def _cleanup_memory_cache(self):
        ...
        if len(self.memory_cache) > 1000:  # Only cleanup if cache is getting large
            now = datetime.now()
            expired_keys = [key for key, (_, expiry) in self.memory_cache.items() if now >= expiry]
    ...
    async def cache_search_results(
        self, query: str, filters: Dict, results: List[Dict], ttl_minutes: int = 30
    ) -> str:
        ...
        try:
            ...
            if success:
                ...
                if retrieved == test_value:
                    ...
                    cache_size = (
                        len(self.memory_cache) if not self.redis_client else "redis_managed"
                    )
                    return {"status": "healthy", "backend": backend, "cache_size": str(cache_size)}
