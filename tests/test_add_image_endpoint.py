...
TEST_IMAGES = [
    ...
    {
        "image_url": (
            "https://arthur.io/img/art/jpg/000173449bb7211f0/m-c-escher/self-portrait-1/large/m-c-escher--self-portrait-1.webp"
        ),
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Total processing time: {result.get('total_processing_time_ms', 0):.1f}ms"
                )
                print(f"   Total request time: {duration:.2f}s")
                return True
            else:
